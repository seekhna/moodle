 ...
 * @return null
 */
function theme_agnostic_page_init(moodle_page $page) {
    $page->requires->jquery();
    $page->requires->js('/theme/agnostic/javascript/theme.js');
}
 ...
 * @return string $css
 */
function theme_agnostic_process_css($css, $theme) {
    ...
    $css = theme_agnostic_set_fontwww($css);
    $css = theme_agnostic_get_pattern_color($css, $theme);
    $css = theme_agnostic_set_customcss($css, $customcss);
 ...
 * @return bool
 */
function theme_agnostic_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    static $theme;

    if (empty($theme)) {
        $theme = theme_config::load('agnostic');
    }
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        if ($filearea === 'logo') {
            ...
            theme_agnostic_serve_css($args[1]);
 ...
 * @return string
 */
function theme_agnostic_serve_css($filename) {
    global $CFG;
    if (!empty($CFG->themedir)) {
        $thestylepath = $CFG->themedir . '/agnostic/style/';
    } else {
        $thestylepath = $CFG->dirroot . '/theme/agnostic/style/';
    }
    ...
    if ((($ifmodifiedsince) && (strtotime($ifmodifiedsince) == $lastmodified)) || $etagheader == $etagfile) {
        theme_agnostic_send_unmodified($lastmodified, $etagfile);
    }
    theme_agnostic_send_cached_css($thestylepath, $filename, $lastmodified, $etagfile);
}
 ...
 *
 */
function theme_agnostic_send_unmodified($lastmodified, $etag) {
 ...
 * @param string $etag
 */
function theme_agnostic_send_cached_css($path, $filename, $lastmodified, $etag) {
    global $CFG;
 ...
 */
function theme_agnostic_set_customcss($css, $customcss) {
 ...
 */
function theme_agnostic_get_html_for_settings(renderer_base $output, moodle_page $page) {
    global $CFG;
    $return = new stdClass;
...
function theme_agnostic_set_fontwww($css) {
    global $CFG, $PAGE;
    ...
    $tag = '[[setting:fontwww]]';
    $theme = theme_config::load('agnostic');
    $css = str_replace($tag, $themewww.'/agnostic/fonts/', $css);
    return $css;
}
 ...
 * @return image $logo
 */
function theme_agnostic_get_logo_url($type='header') {
    global $OUTPUT;
    static $theme;
    if (empty($theme)) {
        $theme = theme_config::load('agnostic');
 ...
 * @return null
 */
function theme_agnostic_render_slideimg($p, $sliname) {
    global $PAGE, $OUTPUT;

    $nos = theme_agnostic_get_setting('numberofslides');
    $i = $p % 3;
    // Get slide image or fallback to default.
    if (theme_agnostic_get_setting($sliname)) {
 ...
 * @return bool
 */
function theme_agnostic_get_setting($setting, $format = true) {
    global $CFG;
 ...
 * @return string
 */
function theme_agnostic_theme_url() {
    global $CFG, $PAGE;
 ...
 */
function theme_agnostic_generate_links($menuname = '') {
    global $CFG, $PAGE;
    $htmlstr = '';
    $menustr = theme_agnostic_get_setting($menuname);
 ...
 * @return array
 */
function theme_agnostic_hidden_courses_ids() {
    global $DB;
    $hcourseids = array();
 ...
 * @return string
 */
function theme_agnostic_strip_html_tags( $text ) {
    $text = preg_replace(
        array(
 ...
 * @return string $out
 */
function theme_agnostic_course_trim_char($str, $n = 500, $endchar = '&#8230;') {
    if (strlen($str) < $n) {
        return $str;
    }
 ...
 * @return string
 */
function theme_agnostic_lang($key = '') {
    $pos = strpos($key, 'lang:');
    if ($pos !== false) {
        list($l, $k) = explode(":", $key);
        if (get_string_manager()->string_exists($k, 'theme_agnostic')) {
            $v = get_string($k, 'theme_agnostic');
            return $v;
        } else {
            return $key;
        }
...
function theme_agnostic_get_pattern_color( $css, $type='') {
    global $OUTPUT , $CFG;
    ...
    $patterncolors = array (
        'default' => array (
            ...
            'collapsed_empty' => $CFG->wwwroot.'/theme/agnostic/pix/default/t/collapsed_empty',
            'collapsed' => $CFG->wwwroot.'/theme/agnostic/pix/default/t/collapsed',
            'collapsed_rtl' => $CFG->wwwroot.'/theme/agnostic/pix/default/t/collapsed_rtl',
            'expanded' => $CFG->wwwroot.'/theme/agnostic/pix/default/t/expanded'
        ),
        ...
        '1' => array (
            ...
            'collapsed_empty' => $CFG->wwwroot.'/theme/agnostic/pix/cs01/t/collapsed_empty.png',
            'collapsed' => $CFG->wwwroot.'/theme/agnostic/pix/cs01/t/collapsed.png',
            'collapsed_rtl' => $CFG->wwwroot.'/theme/agnostic/pix/cs01/t/collapsed_rtl.png',
            'expanded' => $CFG->wwwroot.'/theme/agnostic/pix/cs01/t/expanded.png'
        ),
        '2' => array (
            ...
            'collapsed_empty' => $CFG->wwwroot.'/theme/agnostic/pix/cs02/t/collapsed_empty.png',
            'collapsed' => $CFG->wwwroot.'/theme/agnostic/pix/cs02/t/collapsed.png',
            'collapsed_rtl' => $CFG->wwwroot.'/theme/agnostic/pix/cs02/t/collapsed_rtl.png',
            'expanded' => $CFG->wwwroot.'/theme/agnostic/pix/cs02/t/expanded.png'
        ),
        ...
        '3' => array (
            ...
            'collapsed_empty' => $CFG->wwwroot.'/theme/agnostic/pix/cs03/t/collapsed_empty.png',
            'collapsed' => $CFG->wwwroot.'/theme/agnostic/pix/cs03/t/collapsed.png',
            'collapsed_rtl' => $CFG->wwwroot.'/theme/agnostic/pix/cs03/t/collapsed_rtl.png',
            'expanded' => $CFG->wwwroot.'/theme/agnostic/pix/cs03/t/expanded.png'
        ),
        ...
        '4' => array (
            ...
            'collapsed_empty' => $CFG->wwwroot.'/theme/agnostic/pix/cs04/t/collapsed_empty.png',
            'collapsed' => $CFG->wwwroot.'/theme/agnostic/pix/cs04/t/collapsed.png',
            'collapsed_rtl' => $CFG->wwwroot.'/theme/agnostic/pix/cs04/t/collapsed_rtl.png',
            'expanded' => $CFG->wwwroot.'/theme/agnostic/pix/cs04/t/expanded.png'
        )
    );

    $selectedpattern = theme_agnostic_get_setting('patternselect');
 ...
 * @return type|string
 */
function theme_agnostic_get_hexa($hexa, $opacity) {
    if (!empty($hexa)) {
