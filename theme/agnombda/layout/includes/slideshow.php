...
function slideshow() {
    ...
    $numberofslides = theme_agnostic_get_setting('numberofslides');
    $slideimage = '';
    $visableslide = 0;

    for ($s1 = 1; $s1 <= $numberofslides; $s1++) {
        $slideimage .= theme_agnostic_render_slideimg($s1, 'slide' . $s1 . 'image');
    }
    ...
    if ($slideimage) {
        ...
        if ($numberofslides) {
            ...
            for ($s1 = 1; $s1 <= $numberofslides; $s1++):
                $slidecaption = theme_agnostic_get_setting('slide'.$s1.'caption', true);
                $slidecaption = theme_agnostic_lang($slidecaption);
                $slideimg = theme_agnostic_render_slideimg($s1, 'slide' . $s1 . 'image');
                if (empty($slideimg)) {
                    $slideimg = '';
                }
                $slidebtn = theme_agnostic_get_setting('slide'.$s1.'urltext');
                $slidebtn = theme_agnostic_lang($slidebtn);
                $slidebtnurl = theme_agnostic_get_setting('slide' . $s1 . 'url');
                $icon = "fa-angle-right";
                if (right_to_left()) {
                    $icon = "fa-angle-left";
                }
