 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_agnostic_core_course_renderer extends core_course_renderer {
    ...
    public function frontpage_available_courses() {
        ...
        if (count($rcourseids) > 0) {
            ...
            foreach ($rcourseids as $courseid) {
                ...
                $no = get_config('theme_agnostic', 'patternselect');
    ...
    public function promoted_courses() {
        ...
        $pcourseenable = theme_agnostic_get_setting('pcourseenable');
        if (!$pcourseenable) {
            return false;
        }
        ...
        $featuredids = theme_agnostic_get_setting('promotedcourses');
        $rcourseids = (!empty($featuredids)) ? explode(",", $featuredids) : array();
        if (empty($rcourseids)) {
            return false;
        }

        $hcourseids = theme_agnostic_hidden_courses_ids();
       ...
       // $fcourseids = array_chunk($rcourseids, 6);
        ...
        $promotedtitle = theme_agnostic_get_setting('promotedtitle', 'format_html');
        $promotedtitle = theme_agnostic_lang($promotedtitle);
        ...
        if (!empty($fcourseids)) {
           /* foreach ($fcourseids as $courseids) {
                ...
                foreach ($fcourseids as $courseid) {
                    ...
                    $summary = theme_agnostic_strip_html_tags($course->summary);
                    $summary = theme_agnostic_course_trim_char($summary, 75);
